...
export default function OpenPositionsSlider({ positions }: OpenPositionsSliderProps) {
  ...
  const scrollAmount = 320; // Adjust as needed (e.g., card width + gap)
  ...
  return (
    <div className="relative group">
      <Button
        variant="outline"
        size="icon"
        className="absolute -left-4 top-1/2 -translate-y-1/2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-background/80 hover:bg-background rounded-full shadow-md"
      ...
      <div
        ...
        className="flex overflow-x-auto space-x-6 py-4 scrollbar-hide" // scrollbar-hide might need a plugin or custom CSS
      >
        {positions.map((position) => (
          <div key={position.id} className="flex-shrink-0 w-80"> {/* Ensure cards don't shrink and have a fixed width */}
            <Card className="h-full flex flex-col hover:shadow-lg transition-shadow">
              ...
              <CardContent className="flex-grow">
                <p className="text-sm text-muted-foreground mb-4 line-clamp-3"> {/* Increased line-clamp for more description */}
      ...
      <Button
        variant="outline"
        size="icon"
        className="absolute -right-4 top-1/2 -translate-y-1/2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-background/80 hover:bg-background rounded-full shadow-md"
