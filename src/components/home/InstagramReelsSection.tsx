...
export default function InstagramReelsSection() {
  ...
  const [useRawHtmlEmbed, setUseRawHtmlEmbed] = useState<boolean>(false);
  ...
  const handleReelClick = (reel: InstagramReel) => {
    ...
    if (reelIdFromUrl === specificReelId) {
      setUseRawHtmlEmbed(true);
      setSelectedReelEmbedUrl(null);
    } else {
      const embedUrl = getEmbedUrl(reel.instagramUrl);
      if (embedUrl) {
        setSelectedReelEmbedUrl(embedUrl);
        setUseRawHtmlEmbed(false);
      } else {
        // Fallback for URLs that can't be parsed for iframe, or if getEmbedUrl returns null
        ...
        return; // Prevent modal from opening if we can't get an embed URL
      }
    }
    setIsModalOpen(true);
  };
  ...
  useEffect(() => {
    if (isModalOpen && useRawHtmlEmbed && modalContentRef.current) {
      // Give a brief moment for the DOM to update with dangerouslySetInnerHTML
      const timer = setTimeout(() => {
        if (typeof window.instgrm !== 'undefined' && window.instgrm.Embeds && typeof window.instgrm.Embeds.process === 'function') {
          window.instgrm.Embeds.process();
        } else {
          console.warn("Instagram embed script (window.instgrm) not ready or process function unavailable.");
        }
      }, 100); // 100ms delay, can be adjusted

      return () => clearTimeout(timer);
    }
  }, [isModalOpen, useRawHtmlEmbed]);
    ...
    <>
      ...
      <Dialog open={isModalOpen} onOpenChange={handleModalOpenChange}>
        <DialogContent
            ref={modalContentRef}
            className={cn(
              "border-0 shadow-lg rounded-lg", // Common styles
              useRawHtmlEmbed
                ? "bg-transparent w-auto sm:max-w-xl p-0" // For blockquote, let it determine its own padding/bg
                : "bg-background sm:max-w-[375px] overflow-hidden p-0" // For iframe
            )}
            style={useRawHtmlEmbed ? {} : { aspectRatio: '9/16' }}
        >
          {useRawHtmlEmbed ? (
            <div dangerouslySetInnerHTML={{ __html: specificReelEmbedHtml }} />
            ...
            <iframe
              ...
              className="border-0 block" // Ensure no iframe border
            ></iframe>
          ) : null}
        </DialogContent>
      </Dialog>
